...
class ChartingState extends MusicBeatState
{
	...
	[
		...
		['Hey!', "Plays the \"Hey!\" animation from Bopeebo,\nValue 1: BF = Only Boyfriend, GF = Only Girlfriend, Carol = Only Carol,\nSomething else = Both.\nValue 2: Custom animation duration,\nleave it blank for 0.6s"],
	...
	**/
	...
	function loadHealthIconFromCharacter(char:String) {
		...
		if(!ClientPrefs.OldHDbg) {
			...
			characterPath = 'characters old/' + char + '.json';
		}
		#if MODS_ALLOWED
		...
		#end
		{
			if(!ClientPrefs.OldHDbg) {
				...
				path = Paths.getPreloadPath('characters old/' + Character.DEFAULT_CHARACTER + '.json'); //If a character couldn't be found, change him to BF just to prevent a crash
			}
		}
