...
class AtlasFrameMaker extends FlxFramesCollection
{
	...
	*/
	...
	public static function construct(key:String,?_excludeArray:Array<String> = null, ?noAntialiasing:Bool = false):FlxFramesCollection
	{
		...
		if (Paths.fileExists('images/TextureAtlas/$key/spritemap1.json', TEXT))
		{
		...
		var animationData:AnimationData = Json.parse(Paths.getTextFromFile('images/TextureAtlas/$key/Animation.json'));
		var atlasData:AtlasData = Json.parse(Paths.getTextFromFile('images/TextureAtlas/$key/spritemap.json').replace("\uFEFF", ""));

		var graphic:FlxGraphic = Paths.image('TextureAtlas/$key/spritemap');
